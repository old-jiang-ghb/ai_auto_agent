from xianyu_auto_agent.xianyu_apis import XianYuApis
from utils.xianyu_utils import generate_mid, generate_uuid, trans_cookies, generate_device_id, decrypt
import os, time, asyncio
from loguru import logger
import base64
import json, websockets, random
from xianyu_agent import bot
...
class XianYuLive:
    ...
    # 通过rag的方式回复用户
    async def handle_message_by_rag(self, message_data, websocket):
        """处理所有类型的消息"""
        try:
            # 构建回执包ACK
            try:
            ...
            if not item_id:
                logger.warning("无法获取商品ID")
                return
            # TODO 调用http接口，登录到外部系统中（需要外部系统给一个令牌）
            ...
            if send_user_id == self.myid:
                ...
                # TODO 调用http接口记录 卖家人工回复（也算作ai）
                self.context_manager.add_message_by_chat(chat_id, self.myid, item_id, "assistant", send_message)
                logger.info(f"卖家人工回复 (会话: {chat_id}, 商品: {item_id}): {send_message}")
                return

            logger.info(f"用户: {send_user_name} (ID: {send_user_id}), "
                f"商品: {item_id}, 会话: {chat_id}, 消息: {send_message}")

            # 如果当前会话处于人工接管模式，不进行自动回复
            if self.is_manual_mode(chat_id):
                logger.info(f"🔴 会话 {chat_id} 处于人工接管模式，跳过自动回复")
                # TODO 调用http接口添加用户消息到上下文
                self.context_manager.add_message_by_chat(chat_id, send_user_id, item_id, "user", send_message)
                return
            # 检查是否为带中括号的系统消息
            if self.is_bracket_system_message(send_message):
                ...
                async with websockets.connect(self.base_url, extra_headers=headers) as websocket:
                    ...
                    async for message in websocket:
                        try:
                            ...
                            # TODO 需要增加动态加载配置，判断现在需要怎么处理其他消息
